...
export function ConfigureCropBay({ state, dispatch, navigate }: ConfigureCropBayProps) {
  return (
    <>
      {/* SideNavBar */}
      ...
      <input className="bg-surface-container-lowest border border-slate-700 rounded h-10 pl-10 pr-4 text-on-surface font-body-md focus:border-blue-600 focus:ring-1 focus:ring-blue-600 focus:outline-none transition-colors w-64" placeholder="Search parameters..." type="text"  onChange={(e) => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: e.target.value } })} />
      </div>
      </div>
      {/* Right: Actions & Profile */}
      <div className="flex items-center gap-md">
      {/* Secondary Action */}
      <button className="hidden lg:flex items-center gap-2 h-touch_target px-4 border border-slate-700 rounded text-slate-400 font-label-caps text-label-caps hover:bg-slate-800 transition-colors duration-150 cursor-pointer active:opacity-80" onClick={() => navigate('/settings')}>
                          System Status
                      </button>
      {/* Primary Action */}
      <button className="hidden lg:flex items-center gap-2 h-touch_target px-4 bg-primary-container text-on-primary-container rounded font-label-caps text-label-caps hover:bg-opacity-90 transition-colors duration-150 cursor-pointer active:opacity-80" onClick={() => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: 'Button pressed' } })}>
      <span className="material-symbols-outlined" style={{fontSize: "18px"}}>sync</span>
                          Sync Data
                      </button>
      {/* Icon Actions */}
      <div className="flex items-center gap-unit">
      <button className="h-touch_target w-touch_target flex items-center justify-center text-slate-400 hover:bg-slate-800 rounded transition-colors duration-150 cursor-pointer active:opacity-80 relative" onClick={() => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: 'Button pressed' } })}>
      <span className="material-symbols-outlined">notifications</span>
      <span className="absolute top-2 right-2 w-2 h-2 bg-error rounded-full"></span>
      </button>
      <button className="h-touch_target w-touch_target flex items-center justify-center text-slate-400 hover:bg-slate-800 rounded transition-colors duration-150 cursor-pointer active:opacity-80" onClick={() => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: 'Button pressed' } })}>
      ...
      <a className="hover:text-primary transition-colors" href="#" onClick={() => navigate('/')}>CROP BAYS</a>
      ...
      <input className="h-touch_target bg-surface-container-lowest border border-surface-container-highest rounded px-md font-mono-data text-mono-data text-on-surface opacity-70 cursor-not-allowed" disabled={true} type="text" value="BAY-7G-Alpha-09"  onChange={(e) => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: e.target.value } })} />
      ...
      <select className="appearance-none w-full h-touch_target bg-surface-container-lowest border border-outline-variant rounded px-md pr-10 font-body-md text-body-md text-on-surface focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none transition-colors" onChange={(e) => dispatch({ type: 'UPDATE_SETTINGS', payload: { [e.target.name || e.target.id]: e.target.value } })}>
      ...
      <div className="relative">
      <input className="w-full h-touch_target bg-surface-container-lowest border border-error rounded pl-md pr-10 font-mono-data text-mono-data text-on-surface focus:border-error focus:ring-1 focus:ring-error focus:outline-none transition-colors" type="number" value="85"  onChange={(e) => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: e.target.value } })} />
      ...
      <input className="w-full h-touch_target bg-surface-container-lowest border border-outline-variant rounded px-md font-mono-data text-mono-data text-on-surface focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none transition-colors" type="time" value="06:00"  onChange={(e) => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: e.target.value } })} />
      </div>
      <span className="text-on-surface-variant font-mono-data">to</span>
      <div className="relative flex-1">
      <input className="w-full h-touch_target bg-surface-container-lowest border border-outline-variant rounded px-md font-mono-data text-mono-data text-on-surface focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none transition-colors" type="time" value="22:00"  onChange={(e) => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: e.target.value } })} />
      </div>
      </div>
      </div>
      </div>
      ...
      <button className="h-touch_target px-lg border border-outline-variant rounded text-on-surface font-label-caps text-label-caps hover:bg-surface-container-highest transition-colors focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background outline-none" type="button" onClick={() => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: 'Action: cancel' } })}>
                                  Cancel
                              </button>
      <button className="h-touch_target px-lg bg-primary-container text-on-primary-container rounded font-label-caps text-label-caps hover:bg-opacity-90 transition-colors focus:ring-2 focus:ring-primary focus:ring-offset-2 focus:ring-offset-background outline-none flex items-center gap-sm" type="submit" onClick={() => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: 'Button pressed' } })}>
