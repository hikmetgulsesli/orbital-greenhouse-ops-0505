...
export function CrewMemberProfile({ state, dispatch, navigate }: CrewMemberProfileProps) {
  return (
    <>
      ...
      <button className="font-inter tracking-tight text-sm font-semibold uppercase text-blue-600 cursor-pointer active:opacity-80 hover:bg-slate-800 transition-colors duration-150 px-md py-sm rounded-DEFAULT border border-transparent" onClick={() => dispatch({ type: 'SET_LAST_SYNC', payload: new Date().toISOString() })}>Sync Data</button>
      <div className="flex items-center gap-xs">
      <button className="text-slate-400 hover:bg-slate-800 transition-colors duration-150 p-sm rounded-full cursor-pointer active:opacity-80 flex items-center justify-center h-touch_target w-touch_target" onClick={() => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: 'Button pressed' } })}>
      ...
      <button className="h-10 w-10 rounded-full overflow-hidden border-2 border-blue-600 ml-sm cursor-pointer hover:opacity-80 transition-opacity" onClick={() => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: 'Button pressed' } })}>
      ...
      <a className="bg-slate-800 text-blue-400 border-l-4 border-blue-600 px-4 py-3 flex items-center gap-3 focus-within:ring-2 focus-within:ring-blue-600" href="#" onClick={() => navigate('/')}>
      ...
      <a className="text-slate-400 px-4 py-3 flex items-center gap-3 hover:bg-slate-800 hover:text-slate-100 transition-all duration-200" href="#" onClick={() => navigate('/settings')}>
      <span className="material-symbols-outlined">help_outline</span>
      <span className="font-inter text-xs font-medium uppercase tracking-wider">Support</span>
      </a>
      <a className="text-slate-400 px-4 py-3 flex items-center gap-3 hover:bg-slate-800 hover:text-slate-100 transition-all duration-200" href="#" onClick={() => navigate('/maintenance')}>
      ...
      <button className="h-touch_target w-touch_target flex items-center justify-center rounded-DEFAULT text-on-surface-variant hover:text-on-surface hover:bg-surface-container-highest transition-colors border border-transparent focus:ring-2 focus:ring-primary focus:outline-none focus:border-primary" onClick={() => dispatch({ type: 'ADD_NOTIFICATION', payload: { id: Date.now().toString(), message: 'Button pressed' } })}>
